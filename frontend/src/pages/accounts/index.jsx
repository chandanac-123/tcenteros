...
const Accounts = () => {
  ...
  const revenueLabels = data?.monthly_chart?.map((i) => i.month) || [];
  const incomeData = data?.monthly_chart?.map((i) => i.income) || [];
  const expenseData = data?.monthly_chart?.map((i) => i.expense) || [];
  const navigate = useNavigate();

  const summaryData = [
    {
      title: "Total Income",
      amount: formatIndianCurrency(data?.totals?.income),
    },
    {
      title: "Total Expense",
      amount: formatIndianCurrency(data?.totals?.expense),
    },
    {
      title: "GST Payable",
      amount: formatIndianCurrency(data?.totals?.gst_payable),
    },
    {
      title: "Payroll",
      amount: formatIndianCurrency(data?.totals?.payroll_expense),
    },
  ];
  const colorPalette = [
    {
  ...
  const incomeDataConfig = [
    ...
    },
    {
      ...
      value: formatIndianCurrency(
        data?.income_breakdown?.inventory_sales?.value,
      ),
      color: "#FFCD0F",
    },
    {
  ...
  const expenseDataConfig = [
    ...
    },
    {
      ...
      value: formatIndianCurrency(
        data?.expense_breakdown?.inventory_purchase?.value,
      ),
      color: "#F59E0B",
    },
    {
