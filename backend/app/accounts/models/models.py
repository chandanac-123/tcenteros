from sqlalchemy import Column, String, Integer, Numeric, DateTime, ForeignKey, Enum, Boolean, Text, Index
...
class ChartOfAccounts(Base, AuditMixin):
    ...
    center_id = Column(UUID(as_uuid=True), ForeignKey('center.centers.id'), nullable=False, index=True)  # ADDED
    ...
    # Relationships
    center = relationship("Center", foreign_keys=[center_id])  # ADDED
...
class JournalEntry(Base, AuditMixin):
    ...
    id = Column(Integer, primary_key=True, index=True)
    entry_number = Column(String(50), nullable=False, index=True)  # Removed unique=True
    ...
    center_id = Column(UUID(as_uuid=True), ForeignKey('center.centers.id'), nullable=False, index=True)  # Changed to UUID
    
    status = Column(Enum(EntryStatus), default=EntryStatus.DRAFT, index=True)
...
class TaxLedger(Base, AuditMixin):
    ...
    __table_args__ = (
        ...
        Index('idx_tax_ledger_center', 'center_id'),  # ADDED
        {'schema': 'accounts'}
    )
